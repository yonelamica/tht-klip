...
def transactions_list(request):
    ...
    total_credits = transaction_filter.qs.get_total_credits()
    total_debits = transaction_filter.qs.get_total_debits()
    context = {
...
def transaction_charts(request):
    ...
    income_expense_bar = plot_income_debits_bar_chart(transaction_filter.qs)

    customer_credit_pie = plot_customer_pie_chart(
        transaction_filter.qs.filter(type='credit')
    )
    customer_debit_pie = plot_customer_pie_chart(
        transaction_filter.qs.filter(type='debit')
    )

    context = {
        'filter': transaction_filter,
        'income_expense_barchart': income_expense_bar.to_html(),
        'customer_credit_pie': customer_credit_pie.to_html(),
        'customer_debit_pie': customer_debit_pie.to_html(),
    }
    if request.htmx:
